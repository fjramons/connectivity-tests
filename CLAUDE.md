...
files in `suites/<suite>/`. It generates a normalized test spec, K8s/Compose
...
subfolders: `suites/<suite>/` (CSVs + generated spec, optionally its own
`connectivity-tests.toml` override, and generated server manifests,
standalone script, and run logs nested under `suites/<suite>/outputs/` —
see "Suites" below for the exact subtree). Every entry point requires a
suite, selected via `--suite <name>` (or the equivalent flag on
`run_via_kubectl.sh`) or the `TEST_SUITE` environment variable.
...
uv run src/generate_test_spec.py --suite NAME         # CSV -> suites/NAME/connectivity-test-spec.{yaml,json}
uv run src/generate_server_manifests.py --suite NAME  # spec -> suites/NAME/outputs/manifests/servers/local/<slug>-k8s.yaml + .../remote/<slug>-k8s.yaml (one per unique destination, per direction)
uv run src/generate_standalone_script.py --suite NAME # spec + run_probe.py -> suites/NAME/outputs/standalone/*.sh (self-contained, for the jumphost)
uv run src/generate_report.py --suite NAME            # spec + suites/NAME/outputs/logs/*.json -> suites/NAME/outputs/logs/summary-report.{txt,html}
...
variable, else the `default` suite (`suites/default/`), printed as an `ℹ️`
notice so a run never silently lands in the wrong suite unnoticed. If the
resolved suite's `suites/<name>/` folder doesn't exist, every entry point
...
five entry points, all rooted at one self-contained `suites/<name>/` folder:

- `suites/<name>/` (source CSVs and the generated spec,
  `connectivity-test-spec.{yaml,json}`)
- `suites/<name>/outputs/manifests/servers/local/` (mock listeners for
  `remote_cloud_domain_to_local_cloud_domain` destinations — Local-cloud-
  domain-owned, deployed there directly via `kubectl`)
- `suites/<name>/outputs/manifests/servers/remote/` (mock listeners for
...
- `suites/<name>/outputs/standalone/` (the self-contained jumphost script)
- `suites/<name>/outputs/logs/` (run logs: one `.log` + structured `.json`
...
- optionally `suites/<name>/connectivity-tests.toml` (suite-specific config
  override — see "Config resolution" below)

So switching suites is changing one flag/variable, not several. `ls
suites/` lists the suites that currently exist on disk. Suite/config
...
3. **Jumphost / VM in Local cloud domain** — can reach Remote cloud
   ...
   source, a filtered spec, and `suites/<suite>/connectivity-tests.toml` into heredocs
...
Generated artifacts — the CSVs and generated spec directly under
`suites/<suite>/`, and everything under `suites/<suite>/outputs/`
(manifests, standalone script, run logs) — are **gitignored** (see
`.gitignore`, which lists specific file patterns rather than blanket-
excluding `suites/<suite>/` so that the config file below can stay
tracked) because they contain real internal IPs/topology of Remote cloud
domain and Local cloud domain — they are meant to be regenerated locally
or transferred out-of-band via OneDrive Web, never committed.
`suites/<suite>/connectivity-tests.toml` is the exception:
...
`suites/<suite>/connectivity-tests.toml` if that suite has its own
...
actually need a different `namespace`/pairing/probe calibration get a
`suites/<suite>/connectivity-tests.toml`.

## The spec: single source of truth

`suites/<suite>/connectivity-test-spec.yaml`/`.json` (generated, git-ignored)
...
`generate_report.py --suite NAME` merges every `suites/<suite>/outputs/logs/*.json`
...
`suites/<suite>/outputs/logs/summary-report.{txt,html}`. A test with
...
- Build a minimal JSON spec by hand (a `{"tests": [...]}` list of objects
  ...
  `suites/<suite>/connectivity-test-spec.json` for the exact shape) and run
...
- Validate `run_probe.py batch`'s `.json` companion with
  ...
  `suites/<suite>/` / `suites/<suite>/outputs/logs/` and run
...
`suites/dev-local/`) support both. Every `kind`/`kubectl` call here uses an
